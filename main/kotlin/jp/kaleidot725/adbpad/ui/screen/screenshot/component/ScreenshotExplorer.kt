            ...
            LazyColumn(
                ...
                items(
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Checkbox(
                            checked = screenshot in selectedForDeletion,
                            onCheckedChange = { onToggleScreenshotSelection(screenshot) },
                        )
                        Box(modifier = Modifier.weight(1.0f)) {
                            ThemedContextMenuArea(
                                ...
                                val details =
                                    screenshot.file?.lastModified()?.let { dateFormat.format(Date(it)) }?.let { date ->
                                        screenshot.deviceLabel?.let { "$date · $it" } ?: date
                                    }
                                CommonItemCard(
